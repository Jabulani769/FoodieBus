...
- **Phase 5 — Backend business features**: coupons/promotions, policy-driven cancellation & rescheduling, push notifications, favorites, and multi-stop routes with per-segment pricing and seat-overlap inventory.
  - **Coupons & promotions**: new `Coupon` + `CouponUsage` models (migration `coupons_favorites`); admin CRUD for coupons (SUPER_ADMIN only) plus a public `GET /coupons/:code/validate`; `couponCode` accepted at booking creation and food-order placement with discount recorded on the `Booking`/`FoodOrder` (`couponCode`, `discountAmount`). Redemption validates code/expiry/usage caps inside the caller transaction (via `couponService.redeemCoupon`) with the booking/order id pre-generated so the usage `contextId` is always correct. 15 tests.
  - **Cancellation policy & rescheduling**: `cancelBooking` now applies a policy read from `PlatformSetting.cancellation_policy` (`cancelBeforeHours: 24`, `refundPercent: 50`, `rescheduleFee: 0` defaults; editable via the existing `PUT /admin/settings/:key`) — full refund outside the window, partial inside, a `Refund` row is created, the seat is released and the passenger notified. New `POST /bookings/:id/reschedule` moves a PENDING booking to a different trip/seat, releases the old seat, recomputes the total (new price + fee when inside the window) and clears any coupon. 50 bus tests (incl. a 14-case policy suite).
  - **Push notifications**: `PUSH` added to the `NotificationChannel` enum; new `DeviceToken` model (platform ANDROID/IOS) and `PushProvider` (mock/fcm/apns, configured via `PUSH_PROVIDER`, `FCM_SERVER_KEY`, `APNS_KEY_ID`, `APNS_TEAM_ID`, `APNS_TOPIC`; mock logs only). `POST/GET /notifications/devices`, `DELETE /notifications/devices/:id` (own tokens only), the worker's `sendPush` helper delivers via the provider (no tokens → marked SENT), and `notifyUser` enqueues a PUSH channel when the user has active device tokens. 5 new tests.
  - **Favorites**: new `Favorite` model (userId + optional dish/vendor) with partial unique indexes (migration `favorite_partial_unique`) since Postgres treats NULLs as distinct; add/list/remove own favorites plus `GET /favorites/top` (most-favorited vendors). 8 tests.
  - **Multi-stop routes**: new `RouteStop` model (migration `multi_stop_routes`) — ordered stops with `departureOffsetMinutes` and `segmentPrice` (origin stop must be `0`, first/last stop must match the route's origin/destination); `PUT /bus-routes/:id/stops` replaces a route's stops (admin). Bookings accept `originStopId`/`destinationStopId` and are priced by the sum of segment prices; seat inventory enforces per-segment overlap so two non-overlapping segments can share a seat while any overlapping or full-route booking is rejected (`SEAT_UNAVAILABLE`), and the seat is only released when no active segment booking remains. Trip search matches intermediate stops and returns stops on every result. 17 tests.
  - **Zod error mapping**: the error handler now maps `ZodError` to `400 VALIDATION_ERROR` (previously fell through), and the coupon-list response schema no longer strips the fields.
