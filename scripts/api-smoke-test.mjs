...
step(
  'admin login',
  adminLogin.status === 200 && !!adminLogin.json?.accessToken,
  'POST /auth/login',
);
...
step(
  'health (db+redis ok)',
  health.status === 200 && health.json?.checks?.database === 'ok',
...
const createdStudent = await api('POST', '/users', {
  ...
  body: mkUser(
    studentEmail,
    `+26599${Math.floor(100000 + Math.random() * 900000)}`,
    'STUDENT',
    'Smoke Student',
  ),
});
...
const studentLogin = await api('POST', '/auth/login', {
  body: { identifier: studentEmail, password: 'smokeTest123' },
});
const operatorLogin = await api('POST', '/auth/login', {
  body: { identifier: operatorEmail, password: 'smokeTest123' },
});
step(
  'student login',
  studentLogin.status === 200 && !!studentLogin.json?.accessToken,
  'POST /auth/login',
);
step(
  'operator login',
  operatorLogin.status === 200 && !!operatorLogin.json?.accessToken,
  'POST /auth/login',
);
...
step(
  'list settings',
  settingsBefore.status === 200 && Array.isArray(settingsBefore.json),
  'GET /admin/settings',
);
...
step(
  'upsert cancellation_policy',
  setPolicy.status === 200,
  'PUT /admin/settings/cancellation_policy',
);
...
const bus = await api('POST', '/buses', {
  ...
  body: {
    name: 'Smoke Coach',
    plateNumber: `SMK-${suffix.toUpperCase()}`,
    capacity: 40,
    busType: 'STANDARD',
  },
});
...
step(
  'trip detail has seats',
  tripDetail.status === 200 && Array.isArray(tripDetail.json?.seats),
...
const searched = await api(
  'GET',
  `/trips/search?fromCity=Middle${suffix}&toCity=South${suffix}&date=${new Date(now + 3 * 86400000).toISOString().slice(0, 10)}`,
);
step(
  'search trip by intermediate stop',
  searched.status === 200 && searched.json?.items?.some((t) => t.id === tripId),
  'GET /trips/search (multi-stop)',
);
...
step(
  'booking round-trips to /bookings/me',
  !!roundTripped,
  `GET /bookings/me (total=${roundTripped?.totalAmount})`,
);
step(
  'segment pricing applied (Middle->South = 11000)',
  roundTripped?.totalAmount === '11000',
  `totalAmount=${roundTripped?.totalAmount}`,
);
...
const vendorLogin = await api('POST', '/auth/login', {
  body: { identifier: vendorEmail, password: 'smokeTest123' },
});
...
step(
  ...
  favs.status === 200 &&
    Array.isArray(favs.json?.items) &&
    favs.json.items.some((i) => i.vendorId === vendorId),
  'GET /favorites',
);
const topFavs = await api('GET', '/favorites/top');
step(
  'top favorites',
  topFavs.status === 200 && Array.isArray(topFavs.json?.topVendors),
  'GET /favorites/top',
);
...
step(
  'audit logs readable',
  audit.status === 200,
  `GET /admin/audit-logs (${audit.json?.items?.length ?? 0} entries)`,
);
